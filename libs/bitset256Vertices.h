...
typedef struct bitset {uint64_t parts[4];} bitset;

//  Returns an empty bitset. 
#define EMPTY (bitset) {(uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 0LL}

//  Returns a bitset containing only node.
#define singleton(node) ((node)/64) == 0 ? 	(bitset) {(uint64_t) 1LL << (node), (uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 0LL} : \
						((node)/64) == 1 ? 	(bitset) {(uint64_t) 0LL, (uint64_t) 1LL << (node)%64, (uint64_t) 0LL, (uint64_t) 0LL} : \
						((node)/64) == 2 ? 	(bitset) {(uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 1LL << (node%64), (uint64_t) 0LL} : \
											(bitset) {(uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 1LL << (node%64)}

//  Returns the union of set1 and set2.
#define union(set1, set2) (bitset) {((set1).parts[0] | (set2).parts[0]), \
									((set1).parts[1] | (set2).parts[1]), \
									((set1).parts[2] | (set2).parts[2]), \
									((set1).parts[3] | (set2).parts[3])}
...
#define isEmpty(set) !((set).parts[0] | (set).parts[1] | (set).parts[2] | (set).parts[3]) 

//  Returns the size of the set.
#define size(set) (	__builtin_popcountll((set).parts[0])+ \
					__builtin_popcountll((set).parts[1])+ \
					__builtin_popcountll((set).parts[2])+ \
					__builtin_popcountll((set).parts[3]))

#define equals(set1, set2) (((set1).parts[0] == (set2).parts[0]) &&\
							((set1).parts[1] == (set2).parts[1]) &&\
							((set1).parts[2] == (set2).parts[2]) &&\
							((set1).parts[3] == (set2).parts[3]))
...
#define safeNext128(set, current) ((set).parts[2] ? unsafeNext128(set, current) : safeNext192(set, 191))
#define safeNext192(set, current) ((set).parts[3] ? unsafeNext192(set, current) : -1)
...
#define unsafeNext128(set, current) (__builtin_clzll((set).parts[2]) + ((current) - 128) >= 63 ? safeNext192(set, 191) : \
																								 (current) + __builtin_ctzll((set).parts[2] >> ((current) - 128 + 1)) + 1)
#define unsafeNext192(set, current) (__builtin_clzll((set).parts[3]) + ((current) - 192) >= 63 ? -1 : \
																								 (current) + __builtin_ctzll((set).parts[3] >> ((current) - 192 + 1)) + 1)
...
#define complement(set, sizeOfUniverse) (sizeOfUniverse <= 64 ? (bitset) {~(set).parts[0] << (64-(sizeOfUniverse)) >> (64-(sizeOfUniverse)), (uint64_t) 0LL, (uint64_t) 0LL, (uint64_t) 0LL} : \
										sizeOfUniverse <= 128 ? (bitset) {~(set).parts[0], ~(set).parts[1] << (64 - ((sizeOfUniverse) - 64)) >> (64-((sizeOfUniverse) - 64) ), (uint64_t) 0LL, (uint64_t) 0LL} : \
										sizeOfUniverse <= 192 ?	(bitset) {~(set).parts[0], ~(set).parts[1], ~(set).parts[2] << (64 - ((sizeOfUniverse) - 128)) >>  (64 - ((sizeOfUniverse) - 128)), (uint64_t) 0LL} : \
																(bitset) {~(set).parts[0], ~(set).parts[1], ~(set).parts[2], ~(set).parts[3] << (64 - ((sizeOfUniverse) - 192)) >> (64 - ((sizeOfUniverse) - 192))})
